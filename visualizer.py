...
def visualize_graph_app(graph: main.AirportsGraph, max_vertices: int = 100) -> None:
            ...
            html.Div(
                [
                    ...
                    html.Div(
                        [
                            html.Label(
                                'Search airport: ', style={'marginRight': '10px'}
                            ),
                            dcc.Input(
                                id='search-input',
                                value='',
    ...
    def display_click(
        clickdata: Any,
        max_distance: Any,
        _unused_n_submit: Any,
        search_input: Any,
        _unused_button_state: Any,
        ...
        if ctx.triggered_id == 'submit-button-state':
            ...
            rank_airport_ids = main.AirportsGraph.rank_airports(
                graph, close_airport_ids, 5
            )
            ...
            for node_id in clicked_nodes:
                change_node_marker(
                    clicked_nodes[node_id], {'color': 'black', 'size': 4}
                )
            clicked_nodes.clear()
...
def visualize_graph(graph: main.AirportsGraph, max_vertices: int = 7000):
    ...
    for node in graph_nx.nodes:
        ...
        node_names.append(
            f'Name: {node} | Country: {country} | Global Piece Index: {global_piece_index}'
        )
